"""
Metric registry — loads certified metric definitions (YAML) into typed objects.

A Metric is a reviewed, parameterised SQL template plus the metadata needed to
(a) match a natural-language question to it and (b) render safe SQL from it.
The SQL is owned by humans; the LLM never writes it for a matched metric.
"""
...
@dataclass
class Choice:
    """A discrete parameter (e.g. grain=month|week|day) that drives rendering.
    Each allowed value maps to a dict of template placeholder -> SQL fragment."""
    name: str
    default: str
...
@dataclass
class DateFilter:
    """Declares which column an optional date-range filter applies to."""
    column: str


@dataclass
class Segment:
    """Optional categorical filter, validated against the data_context catalog."""
    column: str
    table: str
...
class Metric:
    ...
    # rendering hints returned to the formatter
...
class Registry:
    ...
    def catalog_for_resolver(self) -> List[dict]:
        """Compact description of every metric for the resolver prompt."""
        out = []
...
def load_registry(definitions_dir: Optional[Path] = None) -> Registry:
    ...
    for path in sorted(directory.glob("*.yaml")):
        ...
        if not raw or "name" not in raw:
            raise ValueError(f"{path.name}: missing required 'name'")
